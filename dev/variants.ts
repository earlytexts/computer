 ...
 * type normaliser applies before Porter stemming.
 *
 * Walks every surface form in the corpus (commonest first), shows what the
 * computer currently normalises it to, and lets you add an override, confirm
 * it, or skip it. An override is keyed on the surface's *folded base* (the
 * same form `normalizeSurface` keys the table on: apostrophes/accents/
 * ligatures folded), and its value is the modern spelling you type.
 *
...
// Surfaces sharing a search bucket (same normalised form), as last built.
const bucket = groupBy(
  vocab.surfaces.map((_, i) => vocab.norms[vocab.surfaceNorm[i]]),
  vocab.cf,
);

// The variant decision, not the Porter stem: whether this surface is mapped to
// a modern spelling, and which other spellings it's searched together with.
// (The mapping is read live; the grouping is the last build's — a mapping you
// add now only regroups on the next `deno task build`.)
const statusLine = (i: number): string => {
  ...
  const members = bucket.get(vocab.norms[vocab.surfaceNorm[i]]) ?? [];
