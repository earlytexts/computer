 ...
 * search); norms are their distinct normalised forms, also sorted.
 * surfaceLemma maps each surface directly to a citation-form string (e.g.
 * "causes" → "cause", "imagined" → "imagine") for use in frequency
 * aggregation and other statistical analyses. The mapping is generated by a
 * suffix-stripping heuristic that checks candidate bases against the corpus
 * surface vocabulary, with curated overrides in lib/lemmas.json for
 * suppletive forms and other systematic exceptions.
 */
export type Vocab = {
  surfaces: string[];
  ...
  cf: number[];
  norms: string[];
  /** surface index -> citation-form lemma string */
  surfaceLemma: string[];
};
...
/**
 * Infer a citation-form lemma for a single surface by trying to strip common
 * inflectional suffixes and checking whether the result exists in the corpus
 * surface vocabulary. Candidates are tested against the actual surface set so
 * the heuristic can never invent a form that doesn't appear in the texts.
...
const buildSurfaceLemma = (surfaces: string[]): string[] => {
  const surfaceSet = new Set(surfaces);
  const overrides = lemmaOverrides as Record<string, string>;
  return surfaces.map((s) => overrides[s] ?? inferLemma(s, surfaceSet));
};
...
export const buildArtefacts = (
  ...
  const vocab: Vocab = {
    ...
    norms,
    surfaceLemma: buildSurfaceLemma(surfaces),
  };
  for (const [surface, tempId] of tempIds) {
