...
import type { Reading } from "@earlytexts/corpus/wire";
 ...
 * 3: the type table is dictionary-driven — per-surface expanded readings replace
 * the single canonical spelling / form bucket / lemma columns. */
 ...
 * topics.json) to 2; the borrowed-members field on EditionRef to 3; the
 * per-posting resolved-reading column in postings.bin to 4.
 */
export const ARTEFACT_VERSION = 4;
...
/**
 * The stored reading value of a posting whose token sits inside exempting markup
 * (person / place / org / citation / language): it indexes verbatim, never
 * normalised, so its reading is the identity of its own surface rather than any
 * index into the register. A real index never reaches this value (a surface has
 * a handful of readings), so it is a safe sentinel.
 */
export const READING_EXEMPT = 0xffffffff;

/**
 * The reading a stored per-posting reading value denotes for `surfaceId`: the
 * surface's reading at that index, or — for the EXEMPT sentinel — the verbatim
 * identity reading of the surface itself. The single mapping the search and
 * statistics sides share, so a stored index means the same everywhere.
 */
export const readingOf = (
  ...
  value === READING_EXEMPT
    ? [{
      spelling: vocab.surfaces[surfaceId],
      lemma: vocab.surfaces[surfaceId],
    }]
    : vocab.readings[surfaceId][value];
...
export type Manifest = {
  ...
  stats: {
    ...
    tokens: number;
    surfaces: number;
    /** Distinct spelling words across every reading (the SPELLING bucket space). */
    spellings: number;
    /** Distinct lemma words across every reading (the FORM/LEMMA bucket space). */
    lemmas: number;
  };
...
/**
 * The type table. Surface forms are the distinct case-folded printed spellings
 * in the corpus, lexicographically sorted (so prefix queries can binary search).
 * Each surface carries its dictionary READINGS — the ways it can be read, from
 * the compiled register (`catalogue/dictionary.json`), or a single identity
 * reading `[[{surface, surface}]]` when the surface is unregistered (best-effort
 * until backfill completes). A reading is a sequence of words (more than one for
 * a contraction, `'tis` → "it is"), each with a modern SPELLING and a citation
 * LEMMA. The two search levels above `exact` and every lemma-keyed statistic
 * derive from these, so the computer holds no spelling or stemming heuristics of
 * its own — the register is the single source of truth.
 *
 * The distinct spelling- and lemma-word bucket tables (and the surface sets that
 * share each) are derived from `readings` at load (see parseArtefacts), not
 * stored. Each posting names, by index into its surface's `readings`, the
 * reading that occurrence resolved to (or the EXEMPT sentinel), so `readingOf`
 * recovers the words of any single occurrence.
 */
export type Vocab = {
  surfaces: string[];
  /** surface index -> its readings, default first (identity when unregistered) */
  readings: Reading[][];
...
export type Postings = {
  ...
  pairs: Uint32Array;
  /** the resolved reading of each posting (index into the surface's readings,
   * or READING_EXEMPT), parallel to `pairs` at half its length */
  readings: Uint32Array;
};
...
export type ServeArtefacts = {
  ...
  /** distinct spelling words across every reading, sorted (query lookup) */
  spellings: string[];
  /** distinct lemma words across every reading, sorted (query lookup) */
  lemmas: string[];
  /** spelling index -> surfaces some reading of which has that spelling word */
  spellingSurfaces: number[][];
  /** lemma index -> surfaces some reading of which has that lemma word */
  lemmaSurfaces: number[][];
...
const postingsBytes = (postings: Postings): Uint8Array =>
  concat([
    asBytes(postings.offsets),
    asBytes(postings.pairs),
    asBytes(postings.readings),
  ]);
...
const readPostings = (bytes: Uint8Array, split: number): Postings => {
  ...
  const total = words[split - 1]; // offsets[surfaces] = posting count (split = surfaces + 1)
  return {
    offsets: words.subarray(0, split),
    pairs: words.subarray(split, split + total * 2),
    readings: words.subarray(split + total * 2, split + total * 3),
  };
};
...
export const parseArtefacts = (files: ArtefactFiles): ServeArtefacts => {
  ...
  const { spellings, lemmas, spellingSurfaces, lemmaSurfaces } = bucketSurfaces(
    vocab,
  );
...
/**
 * The two search-level bucket tables, derived from the per-surface readings:
 * the sorted distinct spelling words and lemma words, and — for each — the
 * surfaces some reading of which uses it (so a query word expands to every
 * surface it could be a reading of). A surface is listed once per bucket even
 * when several of its readings share the word.
 */
const bucketSurfaces = (
  vocab: Vocab,
): {
  spellings: string[];
  lemmas: string[];
  ...
  const spellingsOf = new Map<string, Set<number>>();
  const lemmasOf = new Map<string, Set<number>>();
  const add = (map: Map<string, Set<number>>, word: string, id: number) => {
    const surfaces = map.get(word);
    if (surfaces === undefined) map.set(word, new Set([id]));
    else surfaces.add(id);
  };
  for (let id = 0; id < vocab.surfaces.length; id++) {
    for (const reading of vocab.readings[id]) {
      for (const word of reading) {
        add(spellingsOf, word.spelling, id);
        add(lemmasOf, word.lemma, id);
      }
    }
  }
  const table = (map: Map<string, Set<number>>) => {
    const words = [...map.keys()].sort();
    return {
      words,
      surfaces: words.map((word) => [...map.get(word)!].sort((a, b) => a - b)),
    };
  };
  const spelling = table(spellingsOf);
  const lemma = table(lemmasOf);
  return {
    spellings: spelling.words,
    lemmas: lemma.words,
    spellingSurfaces: spelling.surfaces,
    lemmaSurfaces: lemma.surfaces,
  };
};
